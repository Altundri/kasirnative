									...
									<div id="keranjang">
										...
										<table class="table table-bordered">
											<tr>
												...
												<td><input type="text" readonly="readonly" class="form-control" value="<?php echo $id_tr; ?>" name="id_transaksi"></td>
												...
												if(!empty($bayar))
												{
													...
													if($bayar >= $total)
													{
														...
														for($x=0;$x<$jumlah_dipilih;$x++){
															...
															$d = array($id_transaksi,$id_barang[$x],$id_member[$x],$jumlah[$x],$total[$x],$tgl_input[$x],$periode[$x]);
															$sql = "INSERT INTO nota (id_transaksi,id_barang,id_member,jumlah,total,tanggal_input,periode) VALUES(?,?,?,?,?,?,?)";
															$row = $config->prepare($sql);
															$row->execute($d);

															$transaksi = array($id_transaksi,$total_bayar,$nama_kasir);
															$sql = "INSERT INTO transaksi (id_transaksi,total_belanja,kasir) VALUES(?,?,?)";
															$row = $config->prepare($sql);
															$row->execute($transaksi);
